...
- **NestJS**: 자유도가 높은 Express 대신 계층화 및 모듈화 방식을 사용하는 NestJS로 마이그레이션. 코드 가독성과 유지보수성 향상 기대
...
- 하루마다 업로드 된지 24시간이 지난 이미지를 `temp` 폴더에서 삭제
...
- 프레임워크(Ioc Container)가 대신 의존성 문제를 관리해주기 때문에 코드 구현에 집중 가능
