...
export class AuthService {
    async register(dto: RegisterDto) {
        ...
        if (isExist) throw new CustomError(409, 'Conflict', '이미 존재하는 이메일 입니다');
    ...
    async login(dto: LoginDto) {
        ...
        // I. 404 Not found
        if (!user) throw new CustomError(404, 'Not Found', '가입되지 않은 이메일 입니다');
    ...
    async refresh(token: string) {
        ...
        if (decoded.type !== 'refresh') throw new CustomError(401, 'Unauthorized', '토큰 재발급은 refresh 토큰으로만 가능합니다');
        ...
        if (!validRefresh) throw new CustomError(401, 'Unauthorized', '토큰 유효성 검증에 실패했습니다');
    ...
    async logout(token: string) {
        ...
        if (decoded.type !== 'access') throw new CustomError(401, 'Unauthorized', '로그아웃은 access 토큰으로만 가능합니다');
